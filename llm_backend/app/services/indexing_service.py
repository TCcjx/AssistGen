...
class IndexingService:
    def __init__(self):
        ...
        # 文件 MIME 类型 -> GraphRAG 配置文件。
        # data 目录下只存在这三份配置，分别对应纯文本、PDF、CSV 三种输入，
        # 未列出的类型统一回退到 default_config。
        self.config_mapping = {
    ...
    async def process_file(self, file_info: Dict[str, Any]) -> Dict[str, Any]:
        """处理单个文件的索引构建"""
        # 提前取出路径，保证异常分支里始终有值可引用（否则会变成 NameError，掩盖真实错误）
        ...
        try:
            ...
            # MinerU 是外部 PDF 解析服务，部署地址随环境而异。
            # 允许用环境变量覆盖 data/settings_pdf.yaml 中的默认值，
            # 这样换机器只需改 .env，不必修改仓库内的配置文件。
            if file_type == 'application/pdf':
                mineru_url = os.getenv('MINERU_API_URL')
                mineru_output = os.getenv('MINERU_OUTPUT_DIR')
                if mineru_url:
                    config_overrides['input.mineru_api_url'] = mineru_url
                if mineru_output:
                    config_overrides['input.mineru_output_dir'] = mineru_output
