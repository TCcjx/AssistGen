...
async def create_file_query(
    ...
    路由到该节点说明本轮对话涉及用户上传的文件。文档内容有两条进入途径：
    1. config 中带有 index_id（/api/upload 建好的本地向量索引），走向量检索；
    2. 否则回退到 GraphRAG 本地知识库检索。

    检索到的原文会写入 state.documents，供 check_hallucinations 做事实比对。

    Args:
        state (AgentState): 当前代理状态，包括对话历史。
        config (RunnableConfig): 配置参数，可能包含 index_id。

    Returns:
        Dict[str, Any]: 包含 'messages' 与 'documents' 的字典。
    """
    ...
    if not question:
        return {
            "messages": [AIMessage(content="亲～没有收到您的具体问题呢，可以再说一下想了解什么吗？😊")],
            "documents": [],
        }

    configurable = (config or {}).get("configurable", {}) or {}
    index_id = configurable.get("index_id")
    documents: List[str] = []

    # 途径 1：上传接口建好的本地向量索引
    if index_id:
        try:
            from app.services.rag_chat_service import get_embedding_service

            embedding_service = get_embedding_service()
            await asyncio.to_thread(embedding_service.load_index, index_id)
            results = await embedding_service.search(question, top_k=3)
            documents = [item.get("content", "") for item in results if item.get("content")]
            logger.info(f"file-query retrieved {len(documents)} chunks from index {index_id}")
        except Exception as e:
            logger.error(f"file-query vector retrieval failed: {e}", exc_info=True)

    # 途径 2：回退到 GraphRAG 本地知识库
    if not documents:
        try:
            from app.lg_agent.kg_sub_graph.agentic_rag_agents.components.customer_tools.node import GraphRAGAPI

            search_result = await GraphRAGAPI().query_graphrag(question)
            response_text = (search_result or {}).get("response")
            if response_text:
                documents = [str(response_text)]
        except Exception as e:
            logger.error(f"file-query graphrag retrieval failed: {e}", exc_info=True)

    if not documents:
        return {
            "messages": [AIMessage(content="亲～暂时没有从您上传的文档里找到相关内容呢，可以换个说法再问我一次吗？😊")],
            "documents": [],
        }
    ...
    system_prompt = RAGSEARCH_SYSTEM_PROMPT.format(context="\n\n".join(documents))
    messages = [{"role": "system", "content": system_prompt}] + state.messages
    response = await model.ainvoke(messages)
