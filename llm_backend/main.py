...
import uuid
import os
from app.services.rag_chat_service import RAGChatService, get_embedding_service
from app.services.redis_semantic_cache import shutdown_all_cache_cleanup
import sys
...
async def upload_file(
    ...
    try:
        ...
        # 4. 构建本地向量索引，返回的 index_id 供 /chat-rag 检索使用
        embedding_service = get_embedding_service()
        try:
            embedding_result = await embedding_service.create_embeddings(str(file_path))
        except Exception as e:
            logger.error(f"Embedding index failed for user {user_id}: {str(e)}", exc_info=True)
            embedding_result = {"status": "error", "error": str(e)}

        # 5. 处理 GraphRAG 文件索引（耗时较长，失败不影响上传结果）
        from app.services.indexing_service import IndexingService

        indexing_service = IndexingService()
        index_result = await indexing_service.process_file(file_info)
        
        # 合并结果。status 与 index_id 必须平铺在顶层：
        # 前端是按 X.status === "success" 判断上传结果、并直接读取 X.index_id 的。
        result = {
            **file_info,
            "status": embedding_result.get("status", "error"),
...
async def langgraph_query(request: Request):
    ...
    前端存在两种调用方式：纯文本提问走 JSON body，带图片提问走 multipart 表单。
    这里按 Content-Type 分别解析，两种都支持。
    """
    image: Optional[UploadFile] = None
    ...
    if "multipart/form-data" in content_type:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail="请求体必须是 JSON 或 multipart 表单")
...
class SPAStaticFiles(StaticFiles):
    """让前端 history 路由在刷新时回退到 index.html。"""

    async def get_response(self, path: str, scope):
        if PurePath(path).parts and PurePath(path).parts[0] == "api":
...
async def shutdown_cache_tasks():
    """停止缓存清理任务，避免测试或热重载时留下后台协程。"""
    await shutdown_all_cache_cleanup()


# 最后挂载静态文件，并确保使用绝对路径。SPAStaticFiles 只处理未命中的静态路径，
# API 路由已经先注册，因此不会被这里的回退逻辑吞掉。
